...
const menuItems = [
  { path: '/', icon: 'fa-house-chimney', label: 'Главная' },
  { path: '/explore', icon: 'fa-magnifying-glass', label: 'Поиск' },
  { path: '/messages', icon: 'fa-location-arrow', label: 'Сообщения' },
  { 
    path: '/notifications', 
    icon: 'fa-heart', 
    label: 'Уведомления',
  ...
  { path: '/create', icon: 'fa-square-plus', label: 'Создать' },
  {
    path: currentUser.value ? `/profile/${currentUser.value.username}` : '/profile',
    icon: 'fa-user',
    label: 'Профиль'
  }
]
      ...
      <ul>
        <li v-for="item in menuItems" :key="item.path">
          <router-link :to="item.path" :class="{ active: isActive(item.path) }">
            ...
            <span v-if="item.badge && unreadCount > 0" class="notification-badge">
